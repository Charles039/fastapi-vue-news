...
async def create_user(db:AsyncSession,user_data:UserRequest):
    #先密码加密处理  -> add   (passlib密码加密）(引用security文件内函数）
    hashed_password=security.get_hash_password(user_data.password)
    user=User(username=user_data.username,password=hashed_password)#模型类创建user
    db.add(user)
    await db.commit()
...
async def create_token(db:AsyncSession,user_id:int):
    #生成Token+设置过期时间->查询数据库当前用户是否有Token->有则更新，没有则添加
...
async def update_user(db:AsyncSession,username:str,user_data:UserUpdateRequest):#user_data是pydantic类型，UserUpdateRequest是一个模型类
